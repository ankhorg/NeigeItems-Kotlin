...
public class NbtCompound extends Nbt<RefNbtTagCompound> implements NbtComponentLike {
  ...
  @Override
  public @Nullable bot.inker.bukkit.nbt.NbtCompound getCompound(@NotNull String key, @Nullable bot.inker.bukkit.nbt.NbtCompound def) {
    RefNbtBase value = delegate.get(key);
    return (value instanceof RefNbtTagCompound)
            ? new bot.inker.bukkit.nbt.NbtCompound((RefNbtTagCompound) value)
            : def;
  }

  @Override
  public @NotNull bot.inker.bukkit.nbt.NbtCompound getOrCreateCompound(@NotNull String key) {
    RefNbtBase value = delegate.get(key);
    if (!(value instanceof RefNbtTagCompound)) {
      value = new RefNbtTagCompound();
      set(key, value);
    }
    return new bot.inker.bukkit.nbt.NbtCompound((RefNbtTagCompound) value);
  }
  ...
  @Override
  public @NotNull bot.inker.bukkit.nbt.NbtCompound clone() {
    return new bot.inker.bukkit.nbt.NbtCompound(cloneNms());
  }
  ...
  @Override
  public @Nullable bot.inker.bukkit.nbt.NbtCompound getDeepCompound(@NotNull String key, @Nullable bot.inker.bukkit.nbt.NbtCompound def) {
    RefNbtBase value = getDeepRefNbt(key);
    return value instanceof RefNbtTagCompound
            ? new bot.inker.bukkit.nbt.NbtCompound((RefNbtTagCompound) value)
            : def;
   ...
   * @return this.
   */
  public @NotNull bot.inker.bukkit.nbt.NbtCompound coverWith(@NotNull bot.inker.bukkit.nbt.NbtCompound overlayCompound) {
