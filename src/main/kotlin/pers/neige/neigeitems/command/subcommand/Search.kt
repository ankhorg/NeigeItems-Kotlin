                ...
                setNullExecutor { context ->
                    ...
                    async {
                        ...
                        for (index in (prevItemAmount until prevItemAmount + ConfigManager.config.getInt("ItemList.ItemAmount"))) {
                            ...
                            // 构建信息及物品
                            if (sender is Player) {
                                kotlin.runCatching { ItemManager.getItemStack(id, sender) }.getOrNull()
                                    ?.let { itemStack ->
                                        ...
                                        for ((i, it) in listItemMessageList.withIndex()) {
                                            listItemRaw.append(
                                                ComponentBuilder(it).runCommand("/ni get $id")
                                                    .hoverText(
                                                        ConfigManager.config.getString("Messages.clickGiveMessage")
                                                            ?: ""
                                                    )
                                            )
                        ...
                        if (sender is Player) {
                            listSuffixMessage =
                                listSuffixMessage.replace("{prev}", "!@#$%{prev}!@#$%")
                                    .replace("{next}", "!@#$%{next}!@#$%")
